...
#Info: Modified plugin so you can change filament at a certain height
#Depend: GCode
#Type: postprocess
#Param: pauseLevel(float:5.0) Pause height (mm)
#Param: parkX(float:0) Head park X (mm)
#Param: parkY(float:100) Head park Y (mm)
#Param: moveZ(float:0) Head move Z (mm)
#Param: retractAmount(float:40) Retraction amount (mm)
#Param: pushoutAmount(float:50) Push out amount (mm)

__copyright__ = "Copyright (C) 2013 David Braam - Released under terms of the AGPLv3 License. Modified by Eneko Montero 2017"
import re
	...
	for lIndex in xrange(len(lines)):
		...
		if pauseState == 2:
			...
			if g == 1 or g == 0:# We will do the pause just before printing content. We need to pause from the previous XY position. Not the current.
				...
				if pushoutAmount > retractAmount:
                                        f.write("G1 E-%f F6000\n" % (pushoutAmount - retractAmount))

				#Disable the E steppers
				f.write("M84 E0\n")
				#Wait till the user continues printing
				f.write("M0\n")
				#Push out the filament back, and retract again, the properly primes the nozzle when changing filament.
				f.write("G1 E%f F400\n" % (pushoutAmount))

				#Wait till the user removes the waste
				f.write("M0\n")
